...
async function fetchStockOpnameHistory(noSO) {
  try {
    ...
    // -----------------------------------------------------------------------
    // 2) Cari dulu maksimal 6 NoSO kandidat dengan set lokasi PERSIS sama
    // -----------------------------------------------------------------------
    const headerParams = [
      sixMonthsAgoStr,
      currentDateStr,
    ...
    const headerQuery = `
      ...
      WHERE h.IsBOM = 0
        ...
        AND h.NoSO != ?
      GROUP BY h.NoSO, h.LockedDate, h.Tanggal
      HAVING locations <=> ?   -- set lokasi harus sama persis dengan SO sekarang
    ...
    // -----------------------------------------------------------------------
    // 3) Agregasi per tabel (dibatasi NoSO IN (list 6 header tadi))
    // -----------------------------------------------------------------------
    ...
    //     d.HasNotBeenPrinted != 1
    //     dan tidak ada di tb_stockopname_d_hasil
    ...
    // 3.d) AsetTemuan dari tb_stockopname_non_assets
...
function drawHistoryTableHeader(doc, startX, y, columns, headerHeight = 25) {
  ...
  columns.forEach((col) => {
    ...
    doc.text(col.text, currentX + 3, y + 3, {
      width: col.width - 6,
      align: 'center',
      lineGap: 1,
    });
