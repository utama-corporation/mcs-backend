...
async function renderHasilStockOpnameTable(doc, noSO, withImages = false) {
  ...
  const colX = withImages
    ? [40, 65, 165, 300, 355, 410, 465]
    : [40, 70, 250, 310, 370, 430];
  const colWidths = withImages
    ? [25, 100, 135, 55, 55, 55, 90]
    : [30, 180, 60, 60, 60, 125];
  ...
  for (const [assetCode, parts] of Object.entries(grouped)) {
    ...
    doc
      .fontSize(11)
      .font("Helvetica")
      .text(`${groupIndex}. ${assetCode}`, colX[0] + 5, y);
    y += 15;
    ...
    for (const row of parts) {
      ...
      const rowHeight = Math.max(
        maxTextHeight + 10,
        withImages && imgPath ? IMG_CELL_H + 8 : 0,
      );
          ...
          if (imgPath) {
            ...
            try {
              ...
              const jpegBuffer = await sharp(imgPath)
                ...
                .jpeg({ quality: 85 })
                .toBuffer();
              doc.image(jpegBuffer, colX[i] + pad, y + pad, {
                fit: [maxW, maxH],
              });
            } catch (e) {
    ...
    let totalRowHeight =
      Math.max(
        ...
        ...totalData.map((d, i) =>
          getTextHeight(doc, d, { width: colWidths[COL_SISTEM + i] - 6 }),
        ),
      ) + 10;
