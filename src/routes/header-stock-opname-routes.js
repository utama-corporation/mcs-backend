...
router.post("/no-stock-opname/create", verifyToken, async (req, res) => {
  try {
    ...
    try {
      ...
      const companyIds = await getIdsFromNames(
        pool,
        "tb_company",
        "id_company",
        "company_name",
      ...
      const categoryIds = await getIdsFromNames(
        pool,
        "tb_category_asset",
        "category_code",
        "category_name",
      ...
      const locationIds = await getIdsFromNames(
        pool,
        "tb_location_asset",
        "location_code",
        "location_name",
      ...
      await insertDetails(
        "tb_stockopname_dcategory",
        "IdCategory",
        categoryIds,
      );
      await insertDetails(
        "tb_stockopname_dlocation",
        "IdLocation",
        locationIds,
      );
      ...
      if (IdCompanies && IdCompanies.length > 0) {
        const companyConditions = IdCompanies.map(() => `CompanyName = ?`).join(
          " OR ",
        );
      ...
      if (assets.length > 0) {
       ...
       FROM tb_parts_bom 
       WHERE AssetCode IN (?) AND is_active = 1`,
            [assetCodes],
          );

          console.log(
            `🧾 Parts ditemukan di tb_parts_bom: ${bomAssets.length}`,
          );
          ...
          if (bomAssets.length > 0) {
            ...
            console.log(
              "📋 AssetCodes yang ada di kedua tabel:",
              validAssetCodes,
            );
      ...
      `;
            ...
            console.log(
              "📊 Data yang akan diinsert ke tb_stockopname_bom:",
              bomValues.length,
              "records",
            );
            await pool.query(bomInsertSql, [bomValues]);
            console.log(
              `✅ ${bomAssets.length} parts berhasil diinsert ke tb_stockopname_bom!`,
            );

            // Log AssetCode yang tidak ada di tb_parts_bom
            const missingAssetCodes = assetCodes.filter(
              (code) => !bomAssetCodes.includes(code),
            );
            if (missingAssetCodes.length > 0) {
              console.log(
                "⚠️ AssetCodes tidak ditemukan di tb_parts_bom:",
                missingAssetCodes,
              );
            }
          } else {
            console.log(
              "⚠️ Tidak ada parts BOM yang ditemukan untuk assets ini.",
            );
            console.log(
              "💡 Kemungkinan: AssetCode di tabel asset tidak ada yang terdaftar di tb_parts_bom",
            );
          }
        } else {
    ...
    `;
          ...
          console.log(
            `✅ ${assets.length} assets berhasil diinsert ke tb_stockopname_d!`,
          );
        }
      } else {
...
router.get("/no-stock-opname", verifyToken, async (req, res) => {
  try {
    ...
    const [rows] = await pool.query(
      "SELECT NoSO, Tanggal, IsBOM, LockedDate FROM tb_stockopname_h ORDER BY NoSO DESC",
    );
    ...
    if (rows.length === 0) {
      return res
        .status(404)
        .json({ message: "Tidak ada Jadwal Stock Opname saat ini" });
    }
...
router.delete("/no-stock-opname/delete", verifyToken, async (req, res) => {
  try {
    ...
    if (!NoSO || !Array.isArray(NoSO) || NoSO.length === 0) {
      return res
        .status(400)
        .json({ message: "NoSO harus berupa array dan tidak boleh kosong!" });
    }
    ...
    try {
      ...
      if (result.affectedRows === 0) {
        ...
        return res
          .status(404)
          .json({ message: "Tidak ada data Stock Opname yang ditemukan!" });
      }
