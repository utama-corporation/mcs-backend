...
router.get("/master-company", async (req, res) => {
  try {
    ...
    const [companyRows] = await pool.query(
      "SELECT id_company, company_name FROM tb_company",
    );

    // Ambil daftar kategori asset
    const [categoryRows] = await pool.query(
      "SELECT category_code, category_name FROM tb_category_asset",
    );

    // Ambil daftar lokasi
    const [locationRows] = await pool.query(
      "SELECT location_code, location_name FROM tb_location_asset",
    );
    ...
    if (
      ...
      return res.status(404).json({
        message: "Tidak ada data company atau kategori atau lokasi ditemukan",
      });
    }
...
router.post("/status", verifyToken, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Terjadi kesalahan saat menambahkan status" });
  }
});
...
router.get("/status", async (req, res) => {
  try {
    const [rows] = await pool.query(
      "SELECT * FROM tb_so_status WHERE is_active = 1",
    );
    ...
    res
      .status(500)
      .json({ message: "Terjadi kesalahan saat mengambil data status" });
  }
});
...
router.delete("/status/:id", verifyToken, async (req, res) => {
  ...
  try {
    ...
    if (result.affectedRows === 0) {
      return res
        .status(404)
        .json({ message: "Status tidak ditemukan atau sudah tidak aktif." });
    }
    ...
    res
      .status(500)
      .json({ message: "Terjadi kesalahan saat menghapus status." });
  }
});
...
router.put("/status/:id", verifyToken, async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "Terjadi kesalahan saat mengupdate status." });
  }
});
