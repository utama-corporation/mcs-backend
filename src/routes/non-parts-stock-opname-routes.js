...
router.post("/non-part-stock-opname/create", verifyToken, async (req, res) => {
  try {
    ...
    console.error(
      "❌ Error saat menyimpan ke tb_stockopname_non_parts:",
      error,
    );
    res.status(500).json({
...
// UPDATE Non ASSET
router.put(
  "/non-part-stock-opname/update/:non_part_id",
  verifyToken,
  async (req, res) => {
    try {
      const pool = await connectDb();
      ...
      if (
        image === undefined &&
        non_part_name === undefined &&
        qty === undefined &&
        remark === undefined
      ) {
        return res.status(400).json({
          message:
            "Minimal salah satu dari image, nama part, qty, atau remark harus diisi untuk update!",
        });
      }
