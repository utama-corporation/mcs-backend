...
router.get("/report/:noso/pdf", async (req, res) => {
  try {
    ...
    if (!noSO) {
      return res
        .status(400)
        .json({ message: "Parameter NoSO tidak boleh kosong" });
    }
...
ORDER BY a.LocationAsset, d.AssetCode
    ...
    doc
      .fontSize(16)
      .font("Helvetica-Bold")
      .text("BERITA ACARA STOCK OPNAME", { align: "center" });
    doc.moveDown(1.5);
    ...
    function printAssetTable(items) {
      ...
      doc
        .rect(startX + colNoWidth + colKodeWidth, y, colNamaWidth, headerHeight)
        .stroke();

      doc.text("No", startX + 5, y + 5, {
        width: colNoWidth - 10,
        align: "center",
      });
      doc.text("Kode Aset", startX + colNoWidth + 5, y + 5, {
        width: colKodeWidth - 10,
        align: "center",
      });
      doc.text("Nama Aset", startX + colNoWidth + colKodeWidth + 5, y + 5, {
        width: colNamaWidth - 10,
        align: "center",
      });
      ...
      items.forEach((item, idx) => {
        ...
        const heightKode = doc.heightOfString(assetCodeText, {
          width: colKodeWidth - 10,
        });
        const heightNama = doc.heightOfString(assetNameText, {
          width: colNamaWidth - 10,
        });

        const rowHeight =
          Math.max(heightNo, heightKode, heightNama) + rowPadding * 1.5;
        ...
        doc
          .rect(startX + colNoWidth + colKodeWidth, y, colNamaWidth, rowHeight)
          .stroke();

        // Tulis teks dengan posisi vertikal di padding
        doc.text(noText, startX + 5, y + rowPadding, {
          width: colNoWidth - 10,
          align: "center",
        });
        doc.text(assetCodeText, startX + colNoWidth + 5, y + rowPadding, {
          width: colKodeWidth - 10,
        });
        doc.text(
          assetNameText,
          startX + colNoWidth + colKodeWidth + 5,
          y + rowPadding,
          { width: colNamaWidth - 10 },
        );
    ...
    function printAssetTableWithoutQR(items) {
      ...
      doc
        .rect(
          startX + colNoWidth + colKodeWidth + colNamaWidth,
          y,
          colStatusWidth,
          headerHeight,
        )
      ...
      doc.text(
        "Status",
        startX + colNoWidth + colKodeWidth + colNamaWidth + 5,
        y + 5,
        { width: colStatusWidth - 10, align: "center" },
      );
      ...
      items.forEach((item, idx) => {
        ...
        // Hitung tinggi teks tiap kolom
        const heightNo = doc.heightOfString(noText, { width: colNoWidth - 10 });
        const heightKode = doc.heightOfString(kodeText, {
          width: colKodeWidth - 10,
        });
        const heightNama = doc.heightOfString(namaText, {
          width: colNamaWidth - 10,
        });
        const heightStatus = doc.heightOfString(statusText, {
          width: colStatusWidth - 10,
        });

        const rowHeight =
          Math.max(heightNo, heightKode, heightNama, heightStatus) +
          paddingTop +
          paddingBottom;
        ...
        // Tulis teks
        doc.text(noText, startX + 5, y + paddingTop, {
          width: colNoWidth - 10,
          align: "center",
        });
        doc.text(kodeText, startX + colNoWidth + 5, y + paddingTop, {
          width: colKodeWidth - 10,
        });
        doc.text(
          namaText,
    ...
    function printNonAssetTable(items) {
      ...
      doc
        .rect(
          startX + colNoWidth + colNamaWidth,
          y,
          colKeterangan,
          headerHeight,
        )
      ...
      doc.text("Aset", startX + colNoWidth + 5, y + 5, {
        width: colNamaWidth - 10,
        align: "center",
      });
      doc.text("Keterangan", startX + colNoWidth + colNamaWidth + 5, y + 5, {
        width: colKeterangan - 10,
        align: "center",
      });
      ...
      items.forEach((item, idx) => {
        ...
        const heightKet = doc.heightOfString(ketText, {
          width: colKeterangan - 10,
        });

        const rowHeight =
          Math.max(heightNo, heightNama, heightKet) +
          paddingTop +
          paddingBottom;
        ...
        doc
          .rect(startX + colNoWidth + colNamaWidth, y, colKeterangan, rowHeight)
          .stroke();
        ...
        doc.text(namaText, startX + colNoWidth + 5, y + paddingTop, {
          width: colNamaWidth - 10,
        });
        doc.text(
          ketText,
          startX + colNoWidth + colNamaWidth + 5,
          y + paddingTop,
          { width: colKeterangan - 10 },
        );
    ...
    for (const loc of allLocations) {
      const asetTidakDitemukan = AssetTidakDitemukan.find(
        (g) => g.locationName === loc,
      );
      const asetDitemukanTanpaQR = AssetDitemukanTanpaQR.find(
        (g) => g.locationName === loc,
      );
      const asetTidakTerdaftar = AssetTidakTerdaftar.find(
        (g) => g.locationName === loc,
      );

      const adaAsetTidakDitemukan =
        asetTidakDitemukan && asetTidakDitemukan.assetList.length > 0;
      const adaAsetDitemukanTanpaQR =
        asetDitemukanTanpaQR && asetDitemukanTanpaQR.assetList.length > 0;
      const adaAsetTidakTerdaftar =
        asetTidakTerdaftar && asetTidakTerdaftar.assetList.length > 0;

      if (
        adaAsetTidakDitemukan ||
        adaAsetDitemukanTanpaQR ||
        adaAsetTidakTerdaftar
      ) {
        adaSelisih = true;
        ...
        doc
          .font("Helvetica-Bold")
          .fontSize(10)
          .text(`${locIndex}. Lokasi ${loc}`, doc.page.margins.left + 10);
        doc.moveDown(0.3);
        ...
        if (adaAsetTidakDitemukan) {
          doc
            .font("Helvetica-Bold")
            .fontSize(10)
            .text("A. Aset Tidak Ditemukan", doc.page.margins.left + 20);
        ...
        if (adaAsetDitemukanTanpaQR) {
          ...
          doc
            ...
            .fontSize(10)
            .text(
              "B. Aset Ditemukan Tanpa QR Code",
        ...
        if (adaAsetTidakTerdaftar) {
          ...
          doc
            .font("Helvetica-Bold")
            .fontSize(10)
            .text("C. Aset Temuan", doc.page.margins.left + 20);
    ...
    if (!adaSelisih) {
      doc
        ...
        .fontSize(11)
        .text("Hasil Stock Opname tidak ada selisih.", {
          align: "center",
          valign: "bottom",
        });
    }
    ...
    doc
      .font("Helvetica-Bold")
      .fontSize(12)
      .text("II. Ringkasan Stock Opname", doc.page.margins.left);
    doc.moveDown(0.2);
    ...
    totalScannedAssetRows.forEach((row) => {
      if (
        row.LocationCode &&
        row.LocationName &&
        !locationNameMap[row.LocationCode]
      ) {
        locationNameMap[row.LocationCode] = row.LocationName;
      }
    });
    ...
    nonAssetRows.forEach((row) => {
      if (
        row.location_code &&
        row.LocationName &&
        !locationNameMap[row.location_code]
      ) {
    ...
    const summaryData = lokasiList.map((lokasi, index) => {
      const totalAsset = totalAssetRows.find(
        (row) => row.LocationCode === lokasi,
      );
      const totalScannedAsset = totalScannedAssetRows.find(
        (row) => row.LocationCode === lokasi,
      );
      const notFound = AssetTidakDitemukan.find(
        (l) => l.locationCode === lokasi,
      );
      const tanpaQR = AssetDitemukanTanpaQR.find(
        (l) => l.locationCode === lokasi,
      );
      const tidakTerdaftar = AssetTidakTerdaftar.find(
        (l) => l.locationCode === lokasi,
      );
      ...
      return {
        ...
        totalScannedAsset: totalScannedAsset
          ? totalScannedAsset.TotalAssets
          : 0,
    ...
    function printRingkasanTable(items) {
      ...
      items.forEach((item) => {
        ...
        const rowData = [
          ...
          {
            text: item.totalAssetTersedia.toString(),
            width: colJumlahWidth,
            align: "center",
          },
          {
            text: item.totalScannedAsset.toString(),
            width: colJumlahWidth,
            align: "center",
          },
          {
            text: item.totalNotFound.toString(),
            width: colJumlahWidth,
            align: "center",
          },
          {
            text: item.totalNoQR.toString(),
            width: colJumlahWidth,
            align: "center",
          },
          {
            text: item.totalNonAsset.toString(),
            width: colJumlahWidth,
            align: "center",
          },
        ];
      ...
      // Baris TOTAL
      const totalAssetTersedia = items.reduce(
        (sum, item) => sum + item.totalAssetTersedia,
        0,
      );
      const totalScannedAsset = items.reduce(
        (sum, item) => sum + item.totalScannedAsset,
        0,
      );
      const totalNotFound = items.reduce(
        (sum, item) => sum + item.totalNotFound,
        0,
      );
      const totalNoQR = items.reduce((sum, item) => sum + item.totalNoQR, 0);
      const totalNonAsset = items.reduce(
        (sum, item) => sum + item.totalNonAsset,
        0,
      );
    ...
    function printKomentarBox() {
      ...
      doc
        .font("Helvetica-Bold")
        .fontSize(12)
        .text("III. Tanggapan PIC Terkait Selisih", startX, y);
    ...
    function printJadwalRealisasiTable() {
      ...
      const startY = doc.y;
      const estimatedHeight = 20 + 20 + 20 + 10; // Title + header + row + margin
      if (
      ...
      doc
        .font("Helvetica-Bold")
        .fontSize(12)
        .text("IV. Jadwal VS Realisasi SO", startX, y);
      y += 20; // Move down after title
      ...
      // Check if we have enough space for the row
    ...
    function printTandaTanganBox() {
      ...
      for (let i = 0; i < columnCount; i++) {
        ...
        // Kotak tanda tangan
        doc
          .rect(currentX, updatedY + headerHeight, columnWidth, boxHeight)
          .stroke();
      }
